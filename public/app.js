...
signal.innerHTML = `<div class="emotion-signal-title" style="color:${bColors[topBucket]}">&#129505; Emotional signal detected</div> <div style="font-size:13px;color:${bColors[topBucket]};line-height:1.6"> <strong>Mood zone indicators:</strong> ${[...zones].join(', ')} &middot; <strong>May feel:</strong> ${[...new Set(feelings)].slice(0,3).join(' / ')} &middot; <strong>Suggested bucket:</strong> ${topBucket} </div>`;
}
...
h += `<div class="q"> <label>${cat.label}</label> <input type="text" id="rapport_${cat.id}" placeholder="${cat.placeholder}"> </div>`;
});
...
h += `<div style="background:var(--gray-50);border:1px solid var(--gray-200);border-radius:8px;padding:10px 12px;margin-bottom:8px"> <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px"> <div style="font-size:13px;font-weight:700;color:var(--gray-800)">${t.name}</div> <button onclick="removeTriedStrategy(${i})" style="background:none;border:none;cursor:pointer;color:var(--gray-400);font-size:16px;line-height:1;padding:0">&times;</button> </div> <input type="text" placeholder="What happened when you tried this? (optional)" value="${t.notes}" onchange="updateTriedNote(${i}, this.value)" style="font-size:12px;padding:6px 10px;min-height:auto"> </div>`;
});
...
1. If educator interests are provided, look for genuine connection points between the educator and student (shared interests make the 2x10 and connection strategies more natural).
1. ALL strategies listed in “already tried” MUST be excluded from the primary “try_first” recommendations. They CAN appear in “also_consider” only if the notes suggest they were not implemented with full fidelity.
1. For tried strategies that partially worked, acknowledge what worked and suggest how to build on it.
1. Work WITH the student (Social Discipline Window). Best intervention fits STUDENT, TEACHER, and ENVIRONMENT.
1. Use effort-based and behavior-specific praise only.
...
// ── HEADER ──────────────────────────────────────────────
let h = `<div class="r-header"> <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap"> <div> <div class="r-eyebrow">Be a Scientist &mdash; Put It All Together</div> <div class="r-title">Behavior Intervention Plan</div> <div class="r-meta">${dateStr}</div> </div> <div style="text-align:right;flex-shrink:0"> <div style="background:rgba(255,255,255,.15);border-radius:10px;padding:8px 12px;display:inline-block"> <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;opacity:.7;margin-bottom:3px">Priority</div> <div style="font-size:18px;font-weight:800;color:${p.priority==='high'?'#FCA5A5':p.priority==='moderate'?'#FDE68A':'#86EFAC'}">${p.priority.charAt(0).toUpperCase()+p.priority.slice(1)}</div> </div> </div> </div> <div style="margin-top:10px;margin-bottom:10px">${(p.tags||[]).map(t=>`<span class="tag tag-info">${t}</span>`).join(’’)}</div>
...
alerts.forEach(a => {
h += `<div style="background:${a.bg};border:1px solid ${a.border};border-radius:10px;padding:10px 12px"> <div style="font-size:11px;font-weight:700;color:${a.color};text-transform:uppercase;letter-spacing:.05em;margin-bottom:3px">${a.label}</div> <div style="font-size:12px;color:${a.color};line-height:1.5">${a.text}</div> </div>`;
});
h += `</div>`;
}
...
// Rapport
if (hasRapport) {
h += `<div style="background:#FFFBEB;border:1px solid #FDE68A;border-radius:10px;padding:12px"> <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#B45309;margin-bottom:4px">Rapport</div> ${p.rapport_connection ? `<div style="font-size:12px;color:#92400E;line-height:1.5;margin-bottom:4px">${p.rapport_connection}</div>`: ''} ${p.educator_connection ?`<div style="font-size:12px;color:#92400E;line-height:1.5;font-style:italic">${p.educator_connection}</div>` : ''} </div>`;
}
h += `</div>`;
...
h += `<div class="int-item"> <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:4px;flex-wrap:wrap"> <div class="int-name" style="margin:0">${s.name}</div> <div style="display:flex;gap:6px;align-items:center;flex-shrink:0"> <span class="int-bucket-tag" style="background:${bBg(s.bucket)};color:${bColor(s.bucket)}">${s.bucket}</span> <a class="int-link" href="${BASE+slugify(s.name)}" target="_blank">one-pager &#8599;</a> </div> </div> <div class="int-why">${s.why}</div> ${s.the_moves ? `<div class="int-moves"><strong>How to start:</strong> ${s.the_moves}</div>` : ''} </div>`;
});
h += `</div>`;

// ── ALSO CONSIDER (compact) ──────────────────────────────
if ((p.also_consider||[]).length) {
h += `<div class="rcard"><div class="rcard-title">Also consider</div> <div style="display:flex;flex-direction:column;gap:6px">`;
(p.also_consider||[]).forEach(s => {
h += `<div style="display:flex;align-items:flex-start;gap:10px;padding:8px 0;border-bottom:1px solid var(--gray-100)"> <div style="flex:1"> <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-bottom:3px"> <span style="font-size:13px;font-weight:700;color:var(--gray-800)">${s.name}</span> <span class="int-bucket-tag" style="background:${bBg(s.bucket)};color:${bColor(s.bucket)}">${s.bucket}</span> <a class="int-link" href="${BASE+slugify(s.name)}" target="_blank">one-pager &#8599;</a> </div> <div style="font-size:12px;color:var(--gray-500);line-height:1.5">${s.why}</div> </div> </div>`;
});
h += `</div></div>`;
}

// ── WHAT TO SAY + WHAT NOT TO DO (side by side) ──────────
h += `<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:1rem"> <div class="rcard" style="margin:0;background:#F0FDF4;border-color:#86EFAC"> <div class="rcard-title" style="color:#14532D">What to say</div> <div style="font-size:13px;color:#166534;line-height:1.6;font-style:italic">"${p.what_to_say}"</div> </div>`;
if (p.what_not_to_do) {
h += `<div class="rcard" style="margin:0;background:#FEF2F2;border-color:#FCA5A5"> <div class="rcard-title" style="color:#991B1B">What NOT to do</div> <div style="font-size:13px;color:#7F1D1D;line-height:1.6">${p.what_not_to_do}</div> </div>`;
}
h += `</div>`;

// ── ENVIRONMENT + REVIEW (side by side) ─────────────────
h += `<div style="display:grid;grid-template-columns:2fr 1fr;gap:10px;margin-bottom:1rem"> <div class="rcard" style="margin:0"> <div class="rcard-title">Environmental adjustments</div> <ul style="padding-left:1.2rem;margin:0"> ${(p.environment_fits||[]).map(t=>`<li style="font-size:13px;color:#4B5563;margin-bottom:5px;line-height:1.5">${t}</li>`).join(’’)}
</ul>
</div>
...
const logs = JSON.parse(localStorage.getItem(‘bic_logs’) || ‘[]’);
if (logs.length === 0) {
container.innerHTML = `<div class="log-empty"> <div class="log-empty-icon">&#128203;</div> <div class="log-empty-title">No saved plans yet</div> <div class="log-empty-sub">Complete the 6-step Intervention Tool and click "Save to Log" to store plans here.<br>You can add progress notes to track what worked over time.</div> </div>`;
return;
}
let h = ‘’;
logs.forEach(log => {
...
h += `<div class="log-card" id="log_${log.id}"> <div class="log-card-header"> <div> <div class="log-card-title">Intervention Plan &mdash; ${log.dateStr}</div> <div class="log-card-meta">${log.dateStr} &middot; ${log.bucket} bucket &middot; <span class="status-badge" style="background:${statusColors[log.status]||'#6B7280'}20;color:${statusColors[log.status]||'#6B7280'}">${statusLabels[log.status]||'Active'}</span> </div> </div> <span class="log-card-priority tag ${prTag}">${log.priority}</span> </div> <div class="log-card-summary">${log.summary}</div>`;
...
1. More structured, intensive, or team-based approaches not yet tried
1. Whether a referral, outside services, or formal team meeting is now warranted
1. Any environmental or systemic factors that may be maintaining the behavior
...
let h = `<div class="r-header" style="border-bottom-color:#DC2626"> <div class="r-eyebrow">Going Deeper &mdash; Escalated Intervention Plan</div> <div class="r-title">Deeper Plan &mdash; ${dateStr}</div> <div class="tags"> <span class="tag" style="background:${tierBg};color:${tierColor}">${p.tier_recommendation}</span> <span class="tag tag-high">${p.priority} priority</span> ${(p.tags||[]).map(t=>`<span class="tag tag-info">${t}</span>`).join(’’)}
</div>
...
h += `<div class="int-item"> <div class="int-name">${s.name}</div> <div class="int-meta"><span class="int-bucket-tag" style="background:${bBg(s.bucket)};color:${bColor(s.bucket)}">${s.bucket}</span><a class="int-link" href="${BASE+slugify(s.name)}" target="_blank">view one-pager &#8599;</a></div> <div class="int-why">${s.why}</div> ${s.the_moves ? `<div class="int-moves"><strong>How to start:</strong> ${s.the_moves}</div>` : ''} </div>`;
});
h += `</div>`;
...
h += `<div class="bucket-section" data-bucket="${key}"> <div class="bucket-section-header"> <span class="bucket-section-badge" style="background:${bucket.bg};color:${bucket.color}">${bucket.label}</span> <div> <div class="bucket-section-title" style="color:${bucket.color}">${bucket.label} Strategies</div> <div style="font-size:12px;color:var(--gray-500)">${bucket.desc}</div> </div> </div> <div class="lib-grid">`;
...
if (isAdminMode) {
h += `<div style="background:#EDE9FE;border:1.5px solid var(--purple);border-radius:10px;padding:10px 14px;margin-bottom:1.25rem;font-size:13px;color:var(--purple-dark)"> <strong>Admin:</strong> Click Edit on any resource to update its title, description, or link. Click + Add Resource to add a new one to any section. </div>`;
}

RESOURCES.forEach((section, sIdx) => {
h += `<div class="res-section"> <h3 style="display:flex;align-items:center;justify-content:space-between"> ${section.section} ${isAdminMode ? `<button class="btn btn-sm" style="font-size:11px;color:var(--purple);border-color:var(--purple)" onclick="openAddResource(${sIdx})">+ Add Resource</button>` : ''} </h3> <div class="res-grid">`;
...
let fieldsHtml = fields.map(f => `<div style="margin-bottom:12px"> <label style="font-size:12px;font-weight:600;color:var(--gray-600);display:block;margin-bottom:4px">${f.label}</label> ${f.textarea ?`<textarea id="${f.id}" style="width:100%;padding:8px 12px;border:1.5px solid var(--gray-200);border-radius:8px;font-size:13px;font-family:inherit;min-height:80px;resize:vertical">${f.value}</textarea>`:`<input type=“text” id=”${f.id}” value=”${f.value.replace(/”/g,’"’)}” style=“width:100%;padding:8px 12px;border:1.5px solid var(–gray-200);border-radius:8px;font-size:14px;font-family:inherit”>` } </div>`).join(’’);
...
const parts = key.split(’*’); // step_secId_qId
if (parts.length < 3) return;
const secId = parts[1], qId = parts.slice(2).join(’*’);
...
banner.innerHTML = `Admin Mode — Strategy Library, Resources, Emotions Hub &amp; Step Questions are editable <button onclick="goToPage('library')" style="background:white;color:var(--purple);border:none;border-radius:6px;padding:4px 12px;font-size:12px;font-weight:700;cursor:pointer">Library</button> <button onclick="goToPage('resources')" style="background:white;color:var(--purple);border:none;border-radius:6px;padding:4px 12px;font-size:12px;font-weight:700;cursor:pointer">Resources</button> <button onclick="goToPage('emotions')" style="background:white;color:var(--purple);border:none;border-radius:6px;padding:4px 12px;font-size:12px;font-weight:700;cursor:pointer">Emotions</button> <button onclick="goToPage('tool')" style="background:white;color:var(--purple);border:none;border-radius:6px;padding:4px 12px;font-size:12px;font-weight:700;cursor:pointer">Steps</button> <button onclick="viewFeedback()" style="background:var(--gold);color:var(--purple-dark);border:none;border-radius:6px;padding:4px 12px;font-size:12px;font-weight:700;cursor:pointer">Feedback</button> <span style="font-size:11px;font-weight:400;opacity:.8">Changes sync for all users</span>`;
document.body.appendChild(banner);
...
const card = document.getElementById(’rpc*’ + id);
...
if (!log) return ‘’;
return `\n\nPREVIOUS PLAN FOR THIS STUDENT (${log.dateStr}): Summary: ${log.summary} Strategies that were tried: ${(log.strategies||[]).join(', ')} Previously logged tried strategies: ${log.tried || 'none'} Progress notes: ${(log.progress||[]).map(p=>`${p.date}: ${p.note}`).join('; ') || 'none'} IMPORTANT: Do not repeat the strategies that were already tried unless noting how to implement them differently.`;
} catch(e) { return ‘’; }
}
...
return `<div class="qs-item"> <div class="qs-item-name">${s.name}</div> <span class="qs-item-bucket" style="background:${s.bg};color:${s.color}">${s.bucketLabel}</span> <a class="qs-item-link" href="${url}" target="_blank">one-pager ↗</a> <div class="qs-item-desc">${s.desc.slice(0,90)}${s.desc.length>90?'...':''}</div> </div>`;
}).join(’’);
}
...
if (nextBtn) nextBtn.textContent = *tourStep === TOUR_STEPS.length - 1 ? “Let’s go!” : ‘Next’;
// Dots
const dots = document.getElementById(‘tourDots’);
if (dots) dots.innerHTML = TOUR_STEPS.map((*,i) =>
...
modal.innerHTML = `<div style="background:white;border-radius:16px;padding:1.5rem;max-width:440px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,.3)"> <div style="font-size:16px;font-weight:700;color:var(--gray-800);margin-bottom:4px">Share Your Feedback</div> <div style="font-size:12px;color:var(--gray-500);margin-bottom:1.25rem">Your input helps improve this tool for all educators.</div> <div style="margin-bottom:12px"> <label style="font-size:12px;font-weight:600;color:var(--gray-600);display:block;margin-bottom:6px">How useful was this tool?</label> <div style="display:flex;gap:8px" id="ratingBtns"> ${[1,2,3,4,5].map(n=>`<button onclick="setRating(${n})" id="rb${n}" style="width:40px;height:40px;border-radius:50%;border:2px solid var(--gray-200);background:white;font-size:18px;cursor:pointer;transition:all .15s">${[‘😞’,‘😕’,‘😐’,‘🙂’,‘😄’][n-1]}</button>`).join(’’)}
</div>
</div>
...
try {
const res = await fetch(’/api/overrides-get?key=*all_keys’);
...
modal.style.cssText = ‘position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:1rem’;
modal.innerHTML = `<div style="background:white;border-radius:16px;padding:1.5rem;max-width:440px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,.3)"> <div style="font-size:16px;font-weight:700;color:var(--gray-800);margin-bottom:4px">Before you generate...</div> <div style="font-size:13px;color:var(--gray-500);margin-bottom:1rem">A few fields look incomplete. The plan will be more accurate with this information — but you can still generate now.</div> <ul style="padding-left:1.2rem;margin-bottom:1.25rem"> ${issues.map(i=>`<li style="font-size:13px;color:#D97706;margin-bottom:5px;line-height:1.5">${i}</li>`).join(’’)}
</ul>
<div style="display:flex;gap:8px;justify-content:flex-end">
